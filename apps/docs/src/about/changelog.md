...
* **智能分段 (Smart Paragraph)**：新增"自动分段"功能（默认开启），编辑器将自动把单次换行转换为标准段落，提供更符合公众号写作习惯的呼吸感排版，告别拥挤的文字堆叠。
* **快捷重命名**：文件历史列表支持**双击卡片**直接进行重命名，操作更高效。
* **新主题**：新增 **"极客橙 (Tech Orange)"** 内置主题，主打高信噪比的硬核技术风格，优化了代码块换行与列表排版。

### 优化

* **Markdown 解析**：优化了智能预处理逻辑，更精准地识别换行符，完美兼容 Windows/Mac 不同平台的换行习惯，并修复了代码块内换行可能被误处理的问题。
* **代码体验**：优化了"极客橙"主题下的代码块样式，支持长代码自动换行，移动端阅读更友好。
...
* **案例展示**：新增创作者案例展示页面，汇集优秀排版案例，激发创作灵感。
* **文档集成**：支持 VuePress 文档站点与路由，提供更完善的使用指南。

### 优化

* **界面样式**：优化展示页面样式，提升视觉体验。
...
* **多格式导出**：新增导出为长图、PDF 和 Markdown 功能，满足不同场景下的分享与存档需求。
* **自建图床**：新增 Cloudflare Worker 图片上传服务支持，提供更灵活的图片托管方案。

### 优化

* **API 配置**：更新默认 Cloudflare Worker API 地址。
* **存储配置**：修正 R2 存储桶名称配置，确保存储服务稳定性。
...
* **尾部模板**：编辑器支持尾部模板功能，支持自定义落款，移除旧版微信名片功能。
* **S3 图床支持**：全面支持 S3 兼容协议的图床服务，支持自定义端点、区域等配置。
* **界面重构**：顶部栏新增"更多"菜单，优化设置项布局，操作更加直观。

### 修复

* **Markdown 解析**：修复中文环境下加粗语法在全角符号旁失效的问题（`**粗体**`）。
* **上传体验**：优化 S3 上传错误处理，增加 CORS 配置提示与验证反馈。

<br/>

> [!NOTE] 
> **说明**：以下更新记录为 **咕咚** Fork 后的独立维护版本。
> 
> ----------------------------------------------------------------
